...
To put the performance of our proposed RBF–kernel SVM pipeline into context, we trained a simple Logistic Regression model as a baseline on the same dataset of 918 patients.  
For the baseline we used the integer–encoded features described in Section~\ref{sec:method}: sex, chest pain type, resting ECG, exercise–induced angina, and ST slope were mapped to small integer codes, and no additional feature selection or dimensionality reduction was applied. The model was trained with an 80/20 train–test split and evaluated using the same 5-fold stratified cross-validation protocol as our main model.

The Logistic Regression baseline achieved a hold-out test accuracy of approximately \textbf{0.87} and a mean 5-fold cross-validation accuracy of \textbf{0.85}.  
Our full RBF–SVM pipeline, which combines standardisation, PCA (5 components) and an RBF kernel, achieved a higher mean cross-validation accuracy of \textbf{0.86}, with a mean recall of \textbf{0.90}, precision of \textbf{0.86}, and F1-score of \textbf{0.88} across the five folds. The corresponding ROC–AUC of the SVM model is around \textbf{0.87}, as shown in Figure~\ref{fig:auc}.

Table~\ref{tab:baseline_comparison} summarises the key metrics. Although the absolute gain in accuracy over Logistic Regression is modest (about 1--2 percentage points), the increase in recall and F1-score for the positive (heart disease) class is clinically important, because missing a patient with heart disease is more costly than producing an additional false alarm. This justifies the additional complexity of the SVM-based pipeline over the linear baseline.

\begin{table}[H]
\centering
\begin{tabular}{lcccc}
\toprule
Model & CV Accuracy & Recall (positive) & Precision & F1-score \\
\midrule
Logistic Regression (baseline) & 0.85 & -- & -- & -- \\
RBF--SVM pipeline (ours)       & 0.86 & 0.90 & 0.86 & 0.88 \\
\bottomrule
\end{tabular}
\caption{Comparison between the simple Logistic Regression baseline and our proposed RBF--SVM pipeline (5-fold cross-validation).}
